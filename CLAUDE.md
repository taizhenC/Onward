...
**Update (2026-06-07): figure library grown 3 → 20.** Seventeen new hand-authored `figure_stages` shipped in `lib/figures-data.ts` (each row carries a `Documented:/Interpretive:/Avoid saying:` provenance comment). The as-shipped authoring loop is **author the const + extend `STUB_KEYWORD_MAP` (`lib/match-config.ts`) + add `evals/match.json` cases** — a figure is not done until all three land (a row whose themes no keyword maps to is unmatchable). The `research-figure`/`seed-figure` draft→promote pipeline named below was **never built**; this hand-authoring path is the reality. `themes[]` now draw from a **controlled 21-name vocabulary** (the original 9 + `grief, addiction, shame, illness, disability, dismissed, finding_voice, new_parent_fear, exile, public_failure, self_doubt, bullied`) — reuse exact names, don't fragment with synonyms. Validate with `npm run check-figure` (20/20) and, before seeding, `LLM_PROVIDER=stub PERSISTENCE=memory npm run eval` (stub+memory checks keyword routing for free). At 20 figures the keyword stub still scores **top-1 97.1%, near-miss confusion ≈0%** on `evals/match.json`, so the `>20 figures / eval evidence` gate for the FacetsRAG/vector pipeline (the six-lane architecture below) is **count-reached but NOT evidence-justified** — matching works at this scale; vectors stay deferred until hard-pair confusion actually rises. The 20 figures are **seeded to Supabase** (2026-06-07); the DB serves the full library. Known issue: real-mode (`LLM_PROVIDER=real`) GPT-OSS rerank is **not yet operationally green at full-library scale** — many calls fall back to the keyword scorer and at least one wrong-definitive was observed (a "wrong life" feeling → chandler instead of lee). The 97.1% figure is the **keyword stub**; debugging the real reranker (coverage + wrong-definitive) is the active task before real rerank is trusted as primary.
