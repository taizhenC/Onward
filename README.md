...
| `TELEMETRY_FLOW_BINDING_ENABLED` | Temporary schema/config availability kill switch. Defaults enabled; set `false` only to keep stories on legacy v2, reader progress on the prior CAS, feedback on the legacy RPC, and alternate claims on the legacy RPC while `0011`-`0015` or their configuration is unavailable. This incident mode intentionally loses linked telemetry. |
...
For an existing deployment, use a schema-first rollout. Create a database restore point and validate `0011_transactional_telemetry_outbox.sql`, `0012_match_telemetry_producers.sql`, `0013_story_progress_telemetry.sql`, `0014_story_feedback_telemetry.sql`, and `0015_alternate_request_telemetry.sql` against staging first. Migration `0013` deliberately aborts if any validator-v1 artifact has blank chunks or more than 64 passages; repair or explicitly retire every reported legacy row before retrying, so rollout cannot silently strand an old reader. Keep the currently deployed application in place, apply all five migrations to production, and run `npm run check-db` with the production Supabase URL, service-role key, and a dedicated `TELEMETRY_ID_SECRET`. Only after that gate passes should you deploy the application version that calls `create_story_session_v4`, `issue_match_recovery_flow_v2`, `consume_match_rate_limit_v2`, `acknowledge_story_position_v1`, `submit_story_feedback_v2`, and `claim_alternate_story_flow_v2`; deploying that application first makes new story, recovery, Continue, feedback, or alternate-claim writes fail because those RPCs do not exist yet. Confirm before the window that the running build does not require direct `product_events` writes for a request to succeed, because `0011` revokes both inserts and deletes as soon as it is applied. If a maintenance window is needed, set `STORY_CREATION_ENABLED=false` before the migrations and restore it only after the live-flow check passes. `TELEMETRY_FLOW_BINDING_ENABLED=false` is an explicit temporary escape hatch for schema/config incidents: it keeps story creation on v2 without flow registration or binding, uses the prior owner-scoped progress CAS, retains the legacy feedback RPC, and keeps alternate claims on the legacy RPC, deliberately creating a telemetry gap; it does not replace the migration-first rollout. Remove it or set it to `true` only after `0011`-`0015` and the database gate pass. Migration `0011` is also the prerequisite for any outbox consumer: do not start a consumer until the capture and claim/ack/nack RPCs have been verified.
...
The post-migration database gate must confirm the lifecycle/capture/outbox functions plus `create_story_session_v4`, `issue_match_recovery_flow_v2`, `consume_match_rate_limit_v2`, `acknowledge_story_position_v1`, `submit_story_feedback_v2`, and `claim_alternate_story_flow_v2` are reachable, and that the telemetry-flow and limiter-decision cleanup jobs are active. `npm run check-db` safely resolves those signatures with invalid or nonexistent identifiers and requires expected validation/not-found dispositions before any write, proving the service-role execution grants. It does not inspect `pg_proc`, cron, anon/authenticated revocation, forced RLS, concurrency, or cascades. The following read-only SQL-editor preflight and staging exercises are therefore mandatory, not optional:
...
Rollback compatibility is deliberately narrow. Migrations `0011`-`0015` retain `create_story_session_v3`, `create_story_session_v2`, the prior direct session CAS permissions, the legacy `submit_story_feedback` RPC, and the legacy alternate-claim RPC, so the application can roll back while the new schema remains installed. A pre-0015 rollback loses transactional alternate-request events; a pre-0014 rollback also loses transactional feedback events; a pre-0013 rollback also loses transactional progress/completion events; a v3 rollback loses transactional match-producer completeness; a v2 rollback stops registering/binding new flows and is incident-only. None restores the pre-`0011` direct-write telemetry paths: `0011` revokes direct `product_events` inserts and deletes in favor of `capture_product_event_v1` and `revoke_telemetry_flow_v1`. Do not roll back to an application that requires direct product-event writes; use a rollback build that retains typed capture and privacy deletion through flow revocation, or roll forward. Keep all five migrations installed during the rollback window.
