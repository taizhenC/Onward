...
- **Safe telemetry contract**: exact allowlisted product events and unlinkable operational attempts have no generic metadata or story/input fields; initial intake/match/recovery/artifact plus reader progress/completion now use server-owned producers committed with their domain transition.
...
| `TELEMETRY_FLOW_BINDING_ENABLED` | Temporary schema/config availability kill switch. Defaults enabled; set `false` only to keep stories on the legacy v2 session path and reader progress on the prior owner-scoped CAS while `0011`-`0013` or their configuration is unavailable. This incident mode intentionally loses linked telemetry. |
...
For an existing deployment, use a schema-first rollout. Create a database restore point and validate `0011_transactional_telemetry_outbox.sql`, `0012_match_telemetry_producers.sql`, and `0013_story_progress_telemetry.sql` against staging first. Migration `0013` deliberately aborts if any validator-v1 artifact has blank chunks or more than 64 passages; repair or explicitly retire every reported legacy row before retrying, so rollout cannot silently strand an old reader. Keep the currently deployed application in place, apply all three migrations to production, and run `npm run check-db` with the production Supabase URL, service-role key, and a dedicated `TELEMETRY_ID_SECRET`. Only after that gate passes should you deploy the application version that calls `create_story_session_v4`, `issue_match_recovery_flow_v2`, `consume_match_rate_limit_v2`, and `acknowledge_story_position_v1`; deploying that application first makes new story, recovery, or Continue writes fail because those RPCs do not exist yet. Confirm before the window that the running build does not require direct `product_events` writes for a request to succeed, because `0011` revokes both inserts and deletes as soon as it is applied. If a maintenance window is needed, set `STORY_CREATION_ENABLED=false` before the migrations and restore it only after the live-flow check passes. `TELEMETRY_FLOW_BINDING_ENABLED=false` is an explicit temporary escape hatch for schema/config incidents: it keeps story creation on v2 without flow registration or binding and uses the prior owner-scoped progress CAS, deliberately creating a telemetry gap; it does not replace the migration-first rollout. Remove it or set it to `true` only after `0011`-`0013` and the database gate pass. Migration `0011` is also the prerequisite for any outbox consumer: do not start a consumer until the capture and claim/ack/nack RPCs have been verified.
...
Rollback compatibility is deliberately narrow. Migrations `0011`-`0013` retain `create_story_session_v3`, `create_story_session_v2`, and the prior direct session CAS permissions, so the application can roll back while the new schema remains installed. A pre-0013 rollback loses transactional progress/completion events; a v3 rollback also loses transactional match-producer completeness; a v2 rollback stops registering/binding new flows and is incident-only. None restores the pre-`0011` direct-write telemetry paths: `0011` revokes direct `product_events` inserts and deletes in favor of `capture_product_event_v1` and `revoke_telemetry_flow_v1`. Do not roll back to an application that requires direct product-event writes; use a rollback build that retains typed capture and privacy deletion through flow revocation, or roll forward. Keep all three migrations installed during the rollback window.
