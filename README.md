...
`COMMIT` for that execution. Apply the same whole-file wrapper to `0021`; its
`SET LOCAL` lock timeout and ordered table locks must share one transaction.
