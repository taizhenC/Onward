...
- **Safe telemetry contract**: exact allowlisted product events and unlinkable operational attempts have no generic metadata or story/input fields; initial intake/match/recovery/artifact milestones now use server-owned producers, with artifact and recovery events committed in their domain transaction.
...
| `TELEMETRY_FLOW_BINDING_ENABLED` | Temporary schema/config availability kill switch. Defaults enabled; set `false` only to keep stories on the legacy v2 session path without telemetry-flow registration/binding while `0011`-`0012` or their configuration is unavailable. |
...
1. For a fresh project, apply every file from `supabase/migrations/` in numeric order in the Supabase SQL editor: `0001` → `0002` → `0003` → `0004` → `0005` → `0006` → `0007` → `0008` → `0009` → `0010` → `0011` → `0012`. Before `0003`, enable the **pg_cron** extension and verify `delete from auth.users where false;` runs without a permission error. Apply each migration exactly once and stop on the first error. **`0003` deletes existing development session rows on purpose.** Migration `0009` adds root-only request context plus leased/atomic alternate recovery; `0010` adds typed privacy-safe product/operational telemetry with 30/14-day pruning; `0011` adds the telemetry-flow registry, transactional `create_story_session_v3` binding, typed capture RPC, and leased product-event outbox; `0012` adds transactional match-limiter, recovery, and `create_story_session_v4` artifact producers, including two-day unlinkable limiter-decision replay for ambiguous responses. Pre-`0009` sessions remain deliberately ineligible because their original limits cannot be reconstructed safely.

Before applying `0011`-`0012`, validate the release commit locally:
...
For an existing deployment, use a schema-first rollout. Create a database restore point and validate `0011_transactional_telemetry_outbox.sql` and `0012_match_telemetry_producers.sql` against staging first. Keep the currently deployed v2-calling application in place, apply both migrations to production, and run `npm run check-db` with the production Supabase URL, service-role key, and a dedicated `TELEMETRY_ID_SECRET`. Only after that gate passes should you deploy the application version that calls `create_story_session_v4`, `issue_match_recovery_flow_v2`, and `consume_match_rate_limit_v2`; deploying that application first makes new story/recovery writes fail because those RPCs do not exist yet. Confirm before the window that the running v2 build does not require direct `product_events` writes for a request to succeed, because `0011` revokes both inserts and deletes as soon as it is applied. If a maintenance window is needed, set `STORY_CREATION_ENABLED=false` before the migrations and restore it only after the v4 live-flow check passes. `TELEMETRY_FLOW_BINDING_ENABLED=false` is an explicit temporary escape hatch for schema/config incidents: it keeps story creation on v2 without flow registration or binding, but it does not replace the migration-first rollout. Remove it or set it to `true` only after `0011`-`0012` and the database gate pass. Migration `0011` is also the prerequisite for any outbox consumer: do not start a consumer until the capture and claim/ack/nack RPCs have been verified.
...
Rollback compatibility is deliberately narrow. Migrations `0011`-`0012` retain `create_story_session_v3` and `create_story_session_v2`, so the application can roll back while the new schema remains installed. A v3 rollback keeps flow binding but loses transactional match-producer completeness; a v2 rollback also stops registering/binding new flows and is incident-only. Neither restores the pre-`0011` direct-write telemetry paths: `0011` revokes direct `product_events` inserts and deletes in favor of `capture_product_event_v1` and `revoke_telemetry_flow_v1`. Do not roll back to an application that requires direct product-event writes; use a rollback build that retains typed capture and privacy deletion through flow revocation, or roll forward. Keep both migrations installed during the rollback window.
