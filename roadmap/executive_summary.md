...
| Privacy | Strong ownership/retention foundations, owner-controlled active-database story/account deletion, explicit lifecycle copy, a durable account-level Save State, and a plain-language preview guide | Prove managed-Auth timing, Save/deletion cascades, limiter races, and cron on real Postgres; resolve shared-editorial retention; add provider/backup/PITR restore-deletion evidence and a reviewed market-specific legal notice. |
