...
| P0-11 | In progress | The exact 22-event product union and unlinkable attempt stream reject sensitive/unbounded shapes and reduce failures to closed buckets. Purpose-separated HMAC IDs are retry-stable; signed flows have an immutable 30-day lifetime. Migrations `0011`-`0013` implement the owner/root lifecycle, leased outbox, initial intake/match/recovery/artifact producers, and transactional passage/completion capture. Migration `0014` commits the immutable feedback row with a server-derived `feedback_submitted` event: role comes from root/alternate lineage, verdict comes from the accepted row, and the miss reason never enters telemetry. Exact retries reconcile missing outbox work; changed answers conflict before capture; active-flow capture failures roll back a new feedback row; null/expired flows and the explicit incident switch preserve bounded feedback without fabricated events. Memory mode mirrors these transaction and privacy semantics. Crisis remains stricter than telemetry: resources return before flow/auth/provider/write and no durable crisis event is emitted. | Add transactional alternate producers; add the pre-click flow handoff plus narrow visibility/auth endpoints and one sanitized failure owner; operate a privacy-reviewed dispatcher/reconciler (including in-flight deletion); add temporal aggregates, dashboards, alerts/on-call, minimum-cell rollups, and real Postgres migration/RLS/concurrency/cron/cascade/idempotency proof. |
