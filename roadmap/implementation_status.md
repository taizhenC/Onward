...
| P0-11 | In progress | The exact 22-event product union and unlinkable attempt stream reject sensitive/unbounded shapes and reduce failures to closed buckets. Purpose-separated HMAC IDs are retry-stable; signed flows have an immutable 30-day lifetime. Migrations `0011`-`0013` implement the owner/root lifecycle, leased outbox, initial intake/match/recovery/artifact producers, and transactional passage/completion capture. Migration `0014` commits bounded feedback with a persisted-verdict event and no miss reason. Migration `0015` captures `alternate_requested` only after a durable claim. Migration `0016` adds alternate match calibration and atomically derives the alternate artifact denominator plus first-write-wins terminal outcomes from authoritative state. Fixed entry endpoints capture CTA and first interaction without a tracking SDK. Three exact reader-visibility endpoints capture match-to-preface and Continue-to-passage latency plus first source opening. The match boundary now also proves story-flow anonymous authentication with a short-lived HttpOnly flow challenge plus verified fresh AMR before owner-scoped singleton capture; existing auth and every standalone/account/saved-story auth surface remain silent. Direct story/reload visits without bound timing, invalid auth proofs, cross-origin/malformed calls, and incident-disabled telemetry remain silent. Exact retries reconcile or deduplicate by semantic unit; transaction-coupled capture conflicts roll back their domain transitions, while auth is the documented availability-first exception; and null/revoked flows preserve the product without fabricated telemetry. Memory mode mirrors these transaction and privacy semantics. Crisis resources still return before the match route parses the flow, reads an auth challenge, or writes a crisis event. | Add one sanitized failure owner; operate a privacy-reviewed dispatcher/reconciler (including in-flight deletion); add temporal aggregates, dashboards, alerts/on-call, minimum-cell rollups, and real Postgres migration/RLS/concurrency/cron/cascade/idempotency proof. |
