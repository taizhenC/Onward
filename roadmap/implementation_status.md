...
| P0-11 | In progress | The exact 22-event product union and unlinkable attempt stream reject sensitive/unbounded shapes and reduce failures to closed buckets. Purpose-separated HMAC IDs are retry-stable; signed flow IDs carry an issuance time plus random suffix and cannot reset their 30-day lifetime. Migration `0011` and the memory provider implement issued -> owner-claimed -> root-bound flow state, owner/root reverse lookups, opaque revocation tombstones, root/account/event/outbox cascades, semantic first-write-wins units, and a typed pointer-only outbox with leases, capped backoff, terminal exhaustion, expiry filtering, and ACK/NACK parity. Flow-bearing sessions require an active claimed flow and atomically bind it through v3; a missing flow ID or the explicit binding kill switch deliberately degrades to v2 without a false mapping. Exact committed retries resolve before catalog, recovery-token/rate consumption, matching, or composition. Crisis reaches the server before flow/auth, and the browser posts once before anonymous auth. | Instrument authoritative domain transitions and narrow visibility events without double counting; operate a privacy-reviewed dispatcher/reconciler (including the in-flight deletion rule); add temporal aggregate queries, four launch dashboards, alerts/on-call ownership, minimum-cell rollups, and real Postgres migration/RLS/concurrency/cron/cascade/idempotency proof. |
