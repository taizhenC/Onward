...
| P0-15 | In progress | Strict non-interactive ESLint, isolated production build, deterministic fonts, CI, approved-recipe fail-closed behavior, story kill switch, independent hybrid-composer flag, four-path provider health check, and migrations for immutable StorySpecs/StoryArtifacts, atomic pre-story recovery, current provenance, bounded editorial concerns, immutable completed-story feedback, root-only request context, and leased/atomic alternate finalization exist. All persistence/auth boundaries use one parser that rejects memory in served production even when the build-only override is copied. IP-hash/recovery/alternate secrets share a minimum 32-byte fail-closed parser. The story kill switch blocks new capability issuance/claims without hiding terminal ready results. Migration `0009` is deploy-before-app compatible; `0023` adds strict StorySpec identity, database-owner-anchored exclusive publication/retirement, snapshot CAS, exact schema manifests, closed catalog/role/relation/dependency checks, legacy-RPC revocation, and boolean-only live health. CI executes that migration in embedded PostgreSQL and attacks its terminal authority, full and ordinary lifecycle indexes, generated columns, constraints, relations, rules, triggers, role topology, owners, overloads, ACLs, stale snapshots, and rollback boundaries. CI also runs the story-quality contract suite; rejects mutation/deletion/copying of published evidence, tracked private/run material, unversioned policy changes, and unsynchronized protocol changes; and parses every added history record at its exact content-addressed regular-file path. Story-quality authority surfaces have explicit CODEOWNERS, and main-push CI audits the prior diff. | Complete the route/integration matrix, execute `0023` and the remaining migrations against managed Supabase with preflight/rollback/concurrency/RLS/grant cases, finish broader health/readiness and kill switches, establish protected base-owned story-quality authority, run the real benchmark, and capture green remote CI/deployment evidence. The current production dependency audit also reports high-severity Next.js, PostCSS, and Sharp advisories; upgrade them and make the production audit a CI gate before public release. CODEOWNERS and push audits are not preventive until the repository can require protected pull requests and base-owned checks; the current private-plan limitation remains an external blocker. |
...
rewrite rules, generated publication columns, altered identity, added
terminal-field constraints or indexes, and drifted managed-role topology; then
