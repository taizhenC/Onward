...
| P0-11 | In progress | The exact 22-event product union and unlinkable attempt stream reject sensitive/unbounded shapes and reduce failures to closed buckets. Purpose-separated HMAC IDs are retry-stable; signed flows have an immutable 30-day lifetime. Migration `0011` implements issued -> owner-claimed -> root-bound lifecycle, cascades, semantic idempotency, and a typed leased outbox. Migration `0012` adds server-owned initial intake/match producers, deterministic unlinkable rate-limit scope, atomic match-recovery/clarification capture, and persisted initial artifact capture. Migration `0013` adds server-derived `passage_acknowledged` and final-only `story_completed` in the same CAS transaction, including deterministic replay reconciliation for initial and alternate roles. Ambiguous limiter responses replay one identity-free two-day decision without incrementing twice; drifted recovery retries retain the first calibration row while still issuing a fresh token. Memory mode has matching replay semantics and privacy-shape checks. Null/expired flows progress without fabricated telemetry; the explicit flow-binding incident switch restores the prior owner-scoped CAS and accepts a documented measurement gap. Crisis remains stricter than telemetry: resources return before flow/auth/provider/write and no durable crisis event is emitted. | Add transactional feedback/alternate producers; add the pre-click flow handoff plus narrow visibility/auth endpoints and one sanitized failure owner; operate a privacy-reviewed dispatcher/reconciler (including in-flight deletion); add temporal aggregates, dashboards, alerts/on-call, minimum-cell rollups, and real Postgres migration/RLS/concurrency/cron/cascade/idempotency proof. |
