...
**Authoritative objective:** ship only the launch-critical engineering closure
and external gates in `controlled_public_beta.md`; explicitly deferred work does
not keep the goal open.
...
This is the reusable P0-16/P0-17 presentation seam only. The controlled beta
keeps alternate stories on the exact original stored limits; reader-edited
alternate limits and their commitment protocol are explicitly deferred. Human
accessibility, safety, and editorial evidence remains open.

## Completion audit rule

This ledger preserves evidence for the full long-term roadmap. It no longer
defines the active engineering goal. The current engineering goal stops at the
closure in `controlled_public_beta.md`; P1, P2, and explicitly deferred P0
expansion do not keep it open. External launch gates must still have
authoritative evidence before invitations open. A PR merge, green CI check,
model output, or local smoke result completes only the criteria it directly
covers.
